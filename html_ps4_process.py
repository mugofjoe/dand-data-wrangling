...
# datadir = "data"
datadir = "E:/Google Drive/Backpack/Udacity DAND/04 Data Wrangling/ps4_process_all"
...
def process_file(f):
    ...
    with open("{}/{}".format(datadir, f), "r") as html:
        ...
        for row in rows:
            ...
            if td_set[1].text != 'TOTAL':  # skip TOTAL data 
                info['year'] = int(td_set[0].text)
                info['month'] = int(td_set[1].text)
                info['flights'] = {'domestic': int(td_set[2].text.replace(',','')), 
                                   'international': int(td_set[3].text.replace(',',''))}
